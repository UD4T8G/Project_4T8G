# 디오비스 - Diovis

---

> "..."

## header

---

* [아이기스](#aegis)
* [아스트라페](#astrape)

## main

---

| **직책** | [사자들](../../../core/messengers_core/Messengers_Core.md) |
| :--- | :--- |
| **부서** | [지옥](../../../core/messengers_core/Messengers_Core.md#hell) |
| **이명** | [...](...) |
| **성별** | 남성([디오비스](Diovis_Core.md))/여성([아이기스](#aegis)) |
| **연령대** | 중년기(신)/알 수 없음([사자들](../../../core/messengers_core/Messengers_Core.md)) |

### 특징

---

> "..."

### 능력

---

#### 근거리

---

| **장비** | [아이기스](#aegis) |
| :--- | :--- |
| **특성** | 전사<br>돌진<br>패링<br>군중제어 |
| **기본** | [아이기스](#aegis) 빠르게 휘두르기 후 캐스팅<br>돌진하면서 [아이기스](#aegis) 빠르게 내려찍기 후 캐스팅 |
| **특수** | 캐스팅 후 [아이기스](#aegis) 빠르게 투척 후 오브젝트 반사 후 빠르게 회수 |
| **궁극** | [아이기스](#aegis) 날개를 접고 받은 피해 흡수 채널링 후 날개를 펼쳐 돌진 |

#### 원거리

---

| **장비** | [아스트라페](#astrape) |
| :--- | :--- |
| **특성** | 공성<br>군중제어 |
| **기본** | [아스트라페](#astrape) 빠르게 연속 투척 후 회수<br>캐스팅 후 [아스트라페](#astrape) 빠르게 분열 산탄 투척 후 회수 |
| **특수** | [아스트라페](#astrape) 캐스팅 후 거대한 [아스트라페](#astrape) 빠르게 발사 |
| **궁극** | ... |

#### 거리 조절

---

| **장비** | [아스트라페](#astrape)/[아이기스](#aegis) |
| :--- | :--- |
| **특성** | 스피드스터<br>비행<br>군중제어<br>소환 |
| **기본** | [아이기스](#aegis)로 비행하면서 [아스트라페](#astrape)로 찌르기 후 지나가기 |
| **특수** | [아이기스](#aegis)로 비행하면서 [아스트라페](#astrape) 투척 후 회수 |
| **궁극** | [아이기스](#aegis)로 날아올라 [아스트라페](#astrape) 할복 후 발도하여 공허 폭풍 소환 |

## source

---

### 능력

---

#### <p id="aegis">[아이기스]</p>

---

> import [날개](../../../core/messengers_core/Messengers_Core.md#wings) from [사자들](../../../core/messengers_core/Messengers_Core.md)

* **능력:**
  * 절대 방어
  * 공허 비행
  * 본체 백업
* **설명:**
  * [사자들](../../../core/messengers_core/Messengers_Core.md)의 [날개](../../../core/messengers_core/Messengers_Core.md#wings)이다.
  * [디오비스](Diovis_Core.md)의 권속이다.
  * 방패에 검은 독수리 날개가 장착되어 있다.
    * 등에서 분리하고 접어 방패로 사용할 수 있다.
    * 등에 결합하고 펼쳐 공허 사이를 비행할 수 있다.
 
#### <p id="astrape">[아스트라페]</p>

---

> import [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) from [사자들](../../../core/messengers_core/Messengers_Core.md)

* **능력:**
  * 절대 관통
  * [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) 기폭
* **설명:**
  * [디오비스](Diovis_Core.md)가 쥐고 있는 심판의 벼락이다.
  * 보라색 투창의 형태를 띠고 있으며, 적을 향해 던졌을 때 경로상의 모든 것을 꿰뚫는다.
  * 타겟의 [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) 중첩량에 비례하여 파워가 증가한다.

### 기타

* ...
* ...
* ...