# (Basis Divagans Equorea) Meilichia(메일리키아)

---

> "거기에 거주한 적은 있는데, 거기서 배우거나 가르치거나 하지는 않았어."

## header

---

* [사태극](#sitaegeuk)
  * [융합](#fusion)
    * [순교자들의 융합](#fusion_of_martyrs)
* [신체 개조](#body_modification)
  * [메일리키오스](#meilichios)
    * [메아 쿨파](#mea_culpa)
  * [크루시블](#crucible)
    * [봉인](#seal)
    * [해제](#unseal)
  * [홀스터](#holster)
    * [듀얼 위일드](#dual_wield)
    * [더블 배럴](#double_barrel)
    * [바이폴라](#bipolar)
  * [스캐버드](#scabbard)
    * [듀얼 소드](#dual_sword)
    * [투핸더](#two_hander)
    * [다이폴](#dipole)

## main

---

| **Position** | [사자들](../../../core/messengers_core/Messengers_Core.md) |
| :--- | :--- |
| **Department** | [연옥](../../../core/messengers_core/Messengers_Core.md#purgatorium) |
| **Alias** | [순교자들의 융합](#fusion_of_martyrs) |
| **Gender** | 여성([메일리키아](Meilichia_Core.md))/남성([메일리키오스](#meilichios)) |
| **Age Group** | 소녀기(여신)/알 수 없음([사자들](../../../core/messengers_core/Messengers_Core.md)) |

### Characteristics

---

> "규율에 얽매이지 않고 변칙적이며 가벼운 태도의 [사자들](../../../core/messengers_core/Messengers_Core.md). 공포의 상징과도 같은 연옥의 업무조차도 유희처럼 즐기는 광기를 품고 있다. 하지만 그 이면에는 [사자들](../../../core/messengers_core/Messengers_Core.md)에게 죽음과도 같은 [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect)의 위험을 아슬아슬하게 피해가는 치밀함이 숨어있다. 가장 결정적인 순간, 파트너가 감당해야 할 최악의 페널티 앞에서는 평소의 가벼움을 던져버리고 기꺼이 자신의 형태가 찢겨나가는 것을 감내하는 역설적인 희생정신을 보여준다."

### Abilities

---

#### Melee

---

| **Equipment** | [스캐버드](#scabbard) |
| :--- | :--- |
| **Traits** | 검사<br>돌진<br>패링 |
| **Basic** | [듀얼 소드](#dual_sword) 형태로 회전하면서 빠르게 베기<br>[듀얼 소드](#dual_sword) 형태로 돌진하면서 빠르게 찌르기 |
| **Special** | [투핸더](#two_hander) 형태로 캐스팅 후 돌진하면서 빠르게 베기 |
| **Ultimate** | [다이폴](#dipole)을 던져 오브젝트를 지속적으로 베고 회수<br>충전 후 [다이폴](#dipole)을 손 위에 반중력 회전 비행시키며 지속적으로 오브젝트 요격 |

#### Ranged

---

| **Equipment** | [홀스터](#holster) |
| :--- | :--- |
| **Traits** | 웨펀 마스터<br>탄막<br>저격<br>패링 |
| **Basic** | [듀얼 위일드](#dual_wield) 형태로 지속적으로 빠르게 사격 |
| **Special** | [더블 배럴](#double_barrel) 형태로 캐스팅 후 단일 사격 |
| **Ultimate** | [바이폴라](#bipolar)를 던져 오브젝트를 지속적으로 사격하고 회수<br>[바이폴라](#bipolar)를 손 위에 반중력 회전 비행시키며 지속적으로 오브젝트 요격 |

#### Distance Control

---

| **Equipment** | [신체 개조](#body_modification) |
| :--- | :--- |
| **Traits** | 스피드스터<br>비행<br>돌진<br>탄막<br>패링 |
| **Basic** | 비행하면서 [스캐버드](#scabbard)로 찌르기 후 지나가기<br>회전하면서 전방위 오브젝트를 [스캐버드](#scabbard)로 지속적으로 베기 |
| **Special** | 비행하면서 [홀스터](#holster)로 사격 후 지나가기<br>회전하면서 전방위 오브젝트를 [홀스터](#holster)로 지속적으로 사격 |
| **Ultimate** | 회전 비행하면서 [스캐버드](#scabbard)로 지속적으로 찌르기와 베기를 가하고 [홀스터](#holster)로 지속적으로 사격 후 지나가기 |

## source

---

### Abilities

---

#### <p id="sitaegeuk">[사태극]</p>

---

> import [날개](../../../core/messengers_core/Messengers_Core.md#wings) from [사자들](../../../core/messengers_core/Messengers_Core.md)

* **Ability:**
  * 은신 POV
  * 초공간도약
  * 본체 백업
* **Description:**
  * [사자들](../../../core/messengers_core/Messengers_Core.md)의 [날개](../../../core/messengers_core/Messengers_Core.md#wings)이다.
  * 사람들로부터 자신을 숨기고 관찰할 수 있다.
  * 자신을 다른 좌표로 이동시킨다.
  * 자신의 모든 신체 정보를 저장한다.

#### <p id="fusion">[융합]</p>

---

> import [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) from [사자들](../../../core/messengers_core/Messengers_Core.md)

* **Ability:**
  * 최소 형태를 [사태극](#sitaegeuk)으로 유지
* **Description:**
  * 부활 시스템이다.
  * [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect)이 임계치 이하일 경우 신체를 충전하여 불러올 수 있다.

#### <p id="fusion_of_martyrs">[순교자들의 융합]</p>

---

> import [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) from [사자들](../../../core/messengers_core/Messengers_Core.md)

> import [메일리키오스](#meilichios) from [메일리키아](Meilichia_Core.md)


* **Ability:**
  * [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) 임계 중첩 적용
  * [사태극](#sitaegeuk)을 제외한 모든 형태 소실
  * [메일리키오스](#meilichios) 4세트를 모두 합하여 초대형 [메일리키오스](#meilichios) 소환
* **Description:**
  * 최후의 저항이다.
  * 신체가 모두 소실될때까지 [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect)을 받아들인다.
  * [메일리키오스](#meilichios)의 능력치는 [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) 중첩량에 비례한다.
  * 참수 능력을 사용하고 있지 않을 경우 [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect)이 지속적으로 감소한다.

#### <p id="body_modification">[신체 개조]</p>

---

* **Ability:**
  * [메일리키오스](#meilichios) 장착
  * [크루시블](#crucible) 기동
  * [스캐버드](#scabbard)와 [홀스터](#holster) 동작
* **Description:**
  * [메일리키오스](#meilichios)의 목을 통해 [크루시블](#crucible)의 동력을 [스캐버드](#scabbard)와 [홀스터](#holster)에 전달한다.
  * [스캐버드](#scabbard)와 [홀스터](#holster)를 통해 [메일리키오스](#meilichios)의 머리를 조작할 수 있다.

#### <p id="meilichios">[메일리키오스]</p>

---

* **Ability:**
  * 5세트
  * 소환수
  * 참수
    * 검 형태
    * 총 형태
* **Description:**
  * [메일리키아](Meilichia_Core.md)의 권속이다.
  * 검 2세트, 총 2세트로 구성되어 있다.
  * 참수시켜서 머리를 무기화하고 목을 파이프라인으로 사용할 수 있다.

#### <p id="mea_culpa">[메아 쿨파]</p>

---

> import [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) from [사자들](../../../core/messengers_core/Messengers_Core.md)

* **Ability:**
  * [메일리키오스](#meilichios) 사용 시 [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) 증가
  * [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) 자동 감소
  * [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) 비례 호전성 증가/신체 안정성 감소
* **Description:**
  * [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect) 중첩량이 높아질수록 껍데기가 깎여나가며 글리치를 일으키고, 본질인 무형에 가까워진다.
  * [메일리키오스](#meilichios)를 참수하여 무기로 사용하고 있을 경우 [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect)이 지속적으로 증가한다.
  * [상태이상](../../../core/messengers_core/Messengers_Core.md#status_effect)은 지속적으로 감소하며, 해당 속도는 체내 에너지에 비례한다.

#### <p id="crucible">[크루시블]</p>

---

* **Ability:**
  * 전신 동력 선형 공급
* **Description:**
  * 심장부에 동력원이 장착되어 있다.
  * 등에 동력원과 연결할 수 있는 포트가 장착되어 있다.
    * [메일리키오스](#meilichios)의 꼬리를 연결할 수 있다.
    * [메일리키오스](#meilichios)의 목을 통해 [홀스터](#holster)와 [스캐버드](#scabbard)의 전원을 공급한다.
 
#### <p id="seal">[봉인]</p>

---

> import [메일리키오스](#meilichios) from [메일리키아](Meilichia_Core.md)

* **Ability:**
  * [메일리키오스](#meilichios) 봉인
* **Description:**
  * [해제](#unseal)된 [메일리키오스](#meilichios)를 보관하여 비전투 형태로 전환한다.

#### <p id="unseal">[해제]</p>

---

> import [메일리키오스](#meilichios) from [메일리키아](Meilichia_Core.md)

* **Ability:**
  * [메일리키오스](#meilichios) 해제
* **Description:**
  * [봉인](#seal)된 [메일리키오스](#meilichios)를 [해제](#unseal)하여 전투 형태로 전환한다.

#### <p id="holster">[홀스터]</p>

---

* **Ability:**
  * 2세트
  * 자동 공격
  * 부스터
* **Description:**
  * 다리에 2구가 존재한다.
  * 타겟이 감지될 때마다 자동 사격한다.
  * 뒤로 사격하면 앞으로 추진할 수 있다.

#### <p id="dual_wield">[듀얼 위일드]</p>

---

* **Ability:**
  * [홀스터](#holster) 자동 사격
  * 패링
  * 다중 타겟팅
* **Description:**
  * 총 2세트의 밸런스 타입이다.
  * [홀스터](#holster)에서 꺼내는 동시에 사격한다.
  * 여러 대상을 동시에 제압할 수 있다.
 
#### <p id="double_barrel">[더블 배럴]</p>

---

* **Ability:**
  * 충전
    * 사거리 증가
    * 범위 증가
    * 지속 시간 증가
    * 위력 증가
  * 공격 속도 감소
* **Description:**
  * 총 2세트의 파워 타입이다.
  * 두 개의 총구를 변형해 하나로 결합한다.
  * 공명을 통해 한 번의 사격을 극단적으로 강화한다.


#### <p id="bipolar">[바이폴라]</p>

---

* **Ability:**
  * 반중력 비행
  * 불릿 스톰
  * 공격 속도 증가
* **Description:**
  * 총 2세트의 스피드 타입이다.
  * 두 개의 총신을 변형해 엇갈리게 결합한다.
  * 손 주변에서 회전하거나, 던졌을 때 비행한다.
  * 회전하면서 전방위로 사격한다.

#### <p id="scabbard">[스캐버드]</p>

---

* **Ability:**
  * 2세트
  * 자동 공격
  * 글라이더 형태 전개 및 비행
* **Description:**
  * 등 뒤에 2구가 존재한다.
  * 칼을 휘두르는 팔과도 같다.
  * 무기의 반중력 비행 능력을 활용한다.

#### <p id="dual_sword">[듀얼 소드]</p>

---

* **Ability:**
  * [스캐버드](#scabbard) 자동 공격
  * 발도술
  * 다중 타겟팅
* **Description:**
  * 검 2세트의 밸런스 타입이다.
  * [스캐버드](#scabbard)에서 꺼내는 동시에 공격한다.
  * 여러 대상을 동시에 제압할 수 있다.
 
#### <p id="two_hander">[투핸더]</p>

---

* **Ability:**
  * 충전
    * 범위 증가
    * 사거리 증가
    * 위력 증가
  * 공격 속도 감소
* **Description:**
  * 검 2세트의 파워 타입이다.
  * 두 개의 검을 좌우 양날로 결합한다.
  * 공명을 통해 검의 크기를 증가시킨다.
 
#### <p id="dipole">[다이폴]</p>

---

* **Ability:**
  * 반중력 비행
  * 미라지 소드
  * 공격 속도 증가
* **Description:**
  * 검 2세트의 스피드 타입이다.
  * 두 개의 검을 상하 쌍날로 결합한다.
  * 손 주변에서 회전하거나, 던졌을 때 대상을 향해 날아간다.
  * 회전하면서 주변을 벤다.

### Miscellaneous

---

* [메일리키아](Meilichia_Core.md)의 어원은 고대 그리스의 여신 "Meilichia"에서 유래했다.
  * [메일리키오스](#meilichios)의 어원은 고대 그리스의 신 "Zeus Meilichios"에서 유래했다.
  * 고대 그리스에서 "Meilichios"와 "Meilichia"는 "달래야 하는"으로 번역된다.
    * 이는 신을 달래기 위해 고안된 완곡어법이다.
    * [메일리키아](Meilichia_Core.md)의 융합적인 특징은 다형성에서 유래했다.
      * "Aphrodite Meilichia"
      * 셀레눈테의 "Meilichia"
      * "Mater Meilichia"
      * 기타 등등 
    * "Meilichia"는 현대 그리스어에서 고어이다.
      * "Meilichios"는 "온화한"의 뜻으로만 쓰이고 있다. 
  * [메일리키아](Meilichia_Core.md)의 가벼운 성격은 고대 그리스의 종교 의식이 일상적이었다는 사실에서 유래했다.
* [메일리키아](Meilichia_Core.md)의 특징은 기독교관을 따른다.
  * 상단의 대사는 [4Q531 14(=22) i 4] 에서 유래했다.
    * > "... בגוא בני אנשא הוה ומן בני אנשא לא אלף …"
    * [메일리키아](Meilichia_Core.md)의 가벼운 성격은 문장의 원형따위 신경쓰지 않는다는 점에서도 드러난다.
    * 또한 해당 문장은 화자에 따라 겸손함과 오만함이 결정되는 중의적인 문장이기도 하다.
* [메일리키아](Meilichia_Core.md)의 [날개](../../../core/messengers_core/Messengers_Core.md#wings)에 해당하는 [사태극](#sitaegeuk)은 "십자가"에서 유래했다.
